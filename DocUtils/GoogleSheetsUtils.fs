...
type Sheet(service: SheetsService, spreadsheetId: string, sheetId: string) =
    member _.WriteColumn column offset (data: #seq<string>) =
    ...
    member _.ReadSheet startColumn endColumn offset =
        let range = sheetId + "!" + startColumn + (string offset) + ":" + endColumn + "1000"
    ...
    member this.ReadColumn column offset =
        let column = this.ReadSheet column column offset
        column |> Seq.concat

and Spreadsheet(service: SheetsService, spreadsheetId: string) =
...
and GoogleSheetService(credentialsFileName: string, applicationName: string) =
    let service = 
